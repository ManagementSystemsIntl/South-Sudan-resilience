...
dat <- dat %>%
  mutate(bonding_comp = prin_bond_scrs$PC1,
         bonding_comp_resc = rescale(bonding_comp, c(0,100)))
...
dat <- dat %>%
  mutate(bridging_comp = prin_bridge_scrs$PC1,
         bridging_comp_resc = rescale(bridging_comp, c(0,100)))
...
cohesion_mn <- svyrdat %>%
  summarize(cohesion = survey_mean(cohesion_comp_resc, na.rm=T)) %>%
  mutate(lower=cohesion-1.96*cohesion_se,
         upper=cohesion+1.96*cohesion_se,
         measure="")
...
ggplot(cohesion_mn, aes(cohesion, measure)) + 
  geom_errorbar(aes(xmin=lower, xmax=upper), width=0, color=medium_blue) +
  #geom_hline(yintercept=1, color=light_blue, size=1) +
  geom_segment(aes(x=0, xend=100, y=1, yend=1), color=light_blue) + 
  geom_label(aes(label=round(cohesion,1)), color=medium_blue, size=6, label.padding=unit(.15, "lines")) +
...
ggsave(here("output/viz/social capital/cohesion weighted rescale.png"),
       device="png",
       type="cairo",
       height=1.2,
       width=3)
