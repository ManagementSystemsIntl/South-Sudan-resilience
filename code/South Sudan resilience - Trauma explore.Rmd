...
dat <- dat %>%
  mutate(stress = trauma_3_stress$stress,
         stress_resc = scales::rescale(stress, c(0,100)),
         stress_bin = ifelse(stress>.4999, 1,0),
         stress_quin = ntile(stress, 5))

ggplot(dat, aes(stress_resc)) + 
  geom_bar()

describe(dat$stress_resc)

frq(dat$trauma_feelings)
frq(dat$stress_bin)
