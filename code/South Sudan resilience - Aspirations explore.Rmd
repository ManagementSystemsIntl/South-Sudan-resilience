...
```{r}
asp1 <- svyolr(ordered(aspirations_index2) ~ urban + hh_sex + household_members + as.factor(county),
               method="probit",
               design=svyrdat)
...
```{r}
forw_fa_scrs <- forw_fa$scores %>%
  unlist() %>%
  as.data.frame()

dat <- dat %>%
  mutate(forward_outlook = forw_fa_scrs$ML1)

ggplot(dat, aes(forward_outlook)) + 
  geom_density()
